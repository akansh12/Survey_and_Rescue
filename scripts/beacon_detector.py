...
# ```
	def find_color_contour_centers(self):
		...
		for i in A: # check all 36 RoIs for lit led.
			...
			loc=str(unichr(j//6+65))+str(j%6+1)# formula to extract RoI coordinate from whycon coordinate index.
			j=j+1
			st=""
			x,y,w,h= i # x-y cordinate and its width and length.
			image = self.img #copy of current frame image to perform led detection on.
			cp_image=image[y:y+h,x:x+h] # extract the part of RoI from the image using x-y coordinate and its width and length.
			hsv = cv2.cvtColor(cp_image,cv2.COLOR_BGR2HSV) # convert the extracted part of image to perform hsv detection.

			# next set of codes detect leds using opencv library.
			# red LED
